...
namespace Farmacy.Forme
{
    public partial class IzmeniPakovanjeForm : Form
    {
        ...
        public IzmeniPakovanjeForm(Pakovanje pakovanje)
        {
            InitializeComponent();
            this.pakovanje = pakovanje ?? throw new ArgumentNullException(nameof(pakovanje));
            this.Load += IzmeniPakovanjeForm_Load;
        }
        ...
        private void LoadPakovanjeData()
        {
            if (pakovanje != null)
            {
                ...
                {
                    cmbLek.SelectedItem = pakovanje.Lek.KomercijalniNaziv;
                }

                // Select the shape
                if (pakovanje.Oblik != null)
                {
                    cmbOblik.SelectedItem = pakovanje.Oblik.Naziv;
                }

                txtVelicinaPakovanja.Text = pakovanje.VelicinaPakovanja;
                txtKolicinaAktivne.Text = pakovanje.KolicinaAktivne.ToString();
                ...
                txtPreporuceniRokDana.Text = pakovanje.PreporuceniRokDana?.ToString() ?? "";
            }
        }
        ...
        private void btnSacuvaj_Click(object sender, EventArgs e)
        {
            if (ValidateForm())
            {
                try
                {
                    ...
                    DTOManagerLek.IzmeniPakovanje(dto);
                    MessageBox.Show("Pakovanje je uspešno izmenjeno!", "Uspeh",
                ...
                {
                    MessageBox.Show("Greška pri izmeni pakovanja:\n" + ex.Message, "Greška",
